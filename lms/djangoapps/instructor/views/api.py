...
def certificate_invalidation_view(request, course_id):
    ...
    # Validate request data and return error response in case of invalid data
    try:
        certificate_invalidation_data, certificate = validate_request_data_and_get_certificate(request, course_key)
    except ValueError as error:
        return JsonResponse({'success': False, 'message': error.message}, status=400)
...
def re_validate_certificate(request, course_key, generated_certificate):
    try:
        # Fetch CertificateInvalidation object and deactivate it.
...
def validate_request_data_and_get_certificate(request, course_key):
    """
    ...
    """
    certificate_invalidation = parse_request_data(request)
    user = certificate_invalidation.get("user_name") or certificate_invalidation.get("user_email")
    ...
    if not certificate:
        raise ValueError(_("The student {student} does not have certificate for the course {course}. "
                           "Kindly verify student username/email and the selected course are correct "
                           "and try again.").format(
            student=student.username,
            course=course_key.course,
        ))
    return certificate_invalidation, certificate
